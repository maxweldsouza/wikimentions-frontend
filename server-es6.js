...
let GIT_REV_HASH = PRODUCTION
    ? readFileContentsSync('.GIT_REV_HASH')
...
app.use(
    '/favicon.ico',
    express.static(path.join(__dirname, 'public', 'favicon.ico'), eightdays)
);
app.use(
    '/robots.txt',
    express.static(path.join(__dirname, 'public', 'robots.txt'), nocache)
);
app.use(
    '/assets',
    express.static(path.join(__dirname, 'src', 'assets'), farfuture)
);
app.use(
    '/bundles',
    express.static(path.join(__dirname, SOURCE_DIR, 'bundles'), farfuture)
);
...
const indexHtml = readFileContentsSync(
    path.join(__dirname, SOURCE_DIR, 'index.html')
);
const notFoundHtml = readFileContentsSync(
...
app.get('*', (req, res, next) => {
    ...
    Router.fetchData(api)
        .then(({ apidata, etags }) => {
            ...
            memcached.getMulti(
                [contentKey, modifiedKey, rateLimitKey],
                (err, data) => {
                    if (err) {
                        next(err);
                    }
