...
class ThingPage extends React.Component {
    ...
    render() {
        ...
        const tabHeading = (
            <ul className="tabs text-left" role="tablist">
                {tabs.map(x => {
                    let cls;
                    let aria;
                    if (x === tab) {
        ...
        pageDescription = thing.props.description
            ? `${pageDescription}, ${thing.props.description}`
            : pageDescription;
        let imageUrl;
        if (thing.image) {
